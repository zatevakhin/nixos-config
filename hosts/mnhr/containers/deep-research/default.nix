...
with lib; let
  cfg = config.services.deep-research-compose;
in {
  options.services.deep-research-compose = {
    enable = mkEnableOption "Deep Research Docker Compose service";

    domain = mkOption {
      type = types.str;
      default = "research.homeworld.lan";
      description = "Domain name for the Deep Research service";
    };
