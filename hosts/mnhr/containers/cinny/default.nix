...
with lib; let
  cfg = config.services.cinny-compose;
in {
  options.services.cinny-compose = {
    enable = mkEnableOption "Cinny Docker Compose service";

    domain = mkOption {
      type = types.str;
      default = "cinny.homeworld.lan";
      description = "Domain name for the Cinny service";
    };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.cinny-compose = {
      environment = {
