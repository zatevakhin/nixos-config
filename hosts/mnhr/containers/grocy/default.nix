...
with lib; let
  cfg = config.services.grocy-compose;
in {
  options.services.grocy-compose = {
    enable = mkEnableOption "Grocy Docker Compose service";

    domain = mkOption {
      type = types.str;
      default = "grocy.homeworld.lan";
      description = "Domain name for the Grocy service";
    };
