...
with lib; let
  cfg = config.services.ntfy-compose;
in {
  options.services.ntfy-compose = {
    enable = mkEnableOption "Ntfy Docker Compose service";

    domain = mkOption {
      type = types.str;
      default = "ntfy.homeworld.lan";
      description = "Domain name for the Ntfy service";
    };
